...
echo "=== 2. an already-merged branch says NOTHING TO LAND, not a fake success ==="
run "$ROOT/ff-wt"
assert_rc "exits 0" 0 "$RC"
assert_has "says nothing to land" "NOTHING TO LAND" "$OUT"
case "$OUT" in *"already up to date"*) bad "does not claim a merge happened" "printed 'already up to date' for an empty branch";; *) ok "does not claim a merge happened";; esac
...
echo "=== 5. diverged master is REBASED onto, and both sides survive ==="
R5="$(build diverge)"
...
run "$ROOT/diverge-wt"
assert_rc "exits 0" 0 "$RC"
assert_has "says it rebased" "rebasing" "$OUT"
[ -f "$R5/theirs.md" ] && ok "the card's file landed" || bad "the card's file landed" "theirs.md missing"
[ -f "$R5/mine.md" ] && ok "master's own commit survives" || bad "master's own commit survives" "mine.md gone"
[ "$(git -C "$R5" log --oneline | wc -l | tr -d ' ')" = "3" ] && ok "linear history, no merge commit" || bad "linear history, no merge commit" "got $(git -C "$R5" log --oneline | wc -l | tr -d ' ') commits"

echo "=== 5b. a rebase CONFLICT is refused and the branch is restored ==="
R5B="$(build conflict)"
git -C "$R5B" worktree add -q "$ROOT/conflict-wt" -b card/conf
printf 'from the card\n' > "$ROOT/conflict-wt/same.txt"
git -C "$ROOT/conflict-wt" add -A; git -C "$ROOT/conflict-wt" commit -qm "card edits same.txt"
printf 'from master\n' > "$R5B/same.txt"
git -C "$R5B" add -A; git -C "$R5B" commit -qm "master edits same.txt"
BEFORE_C="$(git -C "$R5B" rev-parse master)"
run "$ROOT/conflict-wt"
assert_rc "exits 1" 1 "$RC"
assert_has "names the conflict" "conflict" "$OUT"
[ "$(git -C "$R5B" rev-parse master)" = "$BEFORE_C" ] && ok "master untouched" || bad "master untouched" "master moved"
[ -z "$(git -C "$ROOT/conflict-wt" status --porcelain)" ] && ok "branch restored (no half-rebase)" || bad "branch restored (no half-rebase)" "$(git -C "$ROOT/conflict-wt" status --porcelain)"
[ "$(git -C "$ROOT/conflict-wt" rev-parse --abbrev-ref HEAD)" = "card/conf" ] && ok "still on its own branch" || bad "still on its own branch" "detached"
